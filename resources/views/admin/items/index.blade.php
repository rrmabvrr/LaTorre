    ...
    <form method="GET" action="{{ route('admin.items.index') }}" class="search-form">
        ...
        <div class="field search-field-category">
            ...
            <select id="category" name="category">
                ...
                <option value="{{ $categoryKey }}" @selected(($filters['category'] ?? '' )===$categoryKey)>
                    {{ $categoryLabel }}
                </option>
                @endforeach
            </select>
...
<div class="card mobile-only" style="margin-bottom: 12px;">
    <div class="mobile-list">
        ...
        <article class="item-card">
            ...
            <p class="item-meta">
                <span class="truncated-text"><strong>Categoria:</strong> {{ $categories[$item->category] ??
                    ucfirst($item->category) }}</span>
                <span class="truncated-text"><strong>Ordem:</strong> {{ $item->display_order }}</span>
                <span class="truncated-text"><strong>Status:</strong> {{ $item->is_available ? 'Disponível' : 'Oculto'
                    }}</span>
            </p>
            ...
            <tr>
                <td>
                    ...
                    <div class="muted texto-curto">
                        @foreach ($item->sizes as $sizeName => $sizeValue)
                        {{ $sizeName }}: R$ {{ number_format((float) $sizeValue, 2, ',', '.') }}@if (! $loop->last) |
                        @endif
                        @endforeach
                    </div>
                    @endif
...
<div class="pagination-wrap">
    ...
    <div class="pagination-desktop desktop-only">
        ...
        @for ($page = $startPage; $page <= $endPage; $page++) @if ($page===$items->currentPage())
