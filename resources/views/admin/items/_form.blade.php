...
<div class="grid">
    ...
    <div class="field" id="priceField"
        style="display: {{ in_array($selectedCategory, $sizeCategories, true) ? 'none' : 'block' }};">
        <label for="price">Preço (R$)</label>
        <input id="price" name="price" type="number" value="{{ old('price', $item->price) }}" min="0" step="0.01"
            @required(!in_array($selectedCategory, $sizeCategories, true))>
    </div>

    <div class="field" id="sizeValuesField"
        style="display: {{ in_array($selectedCategory, $sizeCategories, true) ? 'block' : 'none' }};">
        <label>{{ in_array($selectedCategory, $pizzaCategories, true) ? 'Tamanhos e valores da pizza' : 'Opções e
            valores do suco natural' }}</label>
        <div class="grid"
    ...
    <label for="is_available" style="display:flex;align-items:center;gap:8px;">
        <input id="is_available" name="is_available" type="checkbox" value="1" @checked(old('is_available',
